...
locals {
  zone_id_map = {
    "nazibar.com" = data.aws_route53_zone.website.zone_id
  }
  domain_names = [
    var.website_domain,
  ]
  domain_name = var.website_domain
  bucket_name = var.website_domain

  tags = {
    Project = "nazibar.com"
    Domain  = "nazibar.com"
    Source  = "https://github.com/offbyone/nazibar-com"
  }
}
...
data "aws_iam_policy_document" "deployer" {
  ...
  statement {
    ...
    condition {
      ...
      values   = ["repo:offbyone/nazibar-com:*"]
    }
  }
}

resource "aws_iam_role" "blog_deploy" {
  name               = "nazibar-com-deployer"
  tags               = local.tags
